...
function Wit(opts) {
  ...
  const continueRunActions = (sessionId, message, prevContext, i, confidenceThreshold) => {
      ...
      if (json.type === 'msg') {
        ...
        return actions.send(request, response).then((ctx) => {
          ...
          return this.converse(sessionId, null, prevContext).then(
            continueRunActions(sessionId, message, prevContext, i - 1, confidenceThreshold)
          );
        });
        ...
        return actions[action](request).then((ctx) => {
          ...
          return this.converse(sessionId, null, nextContext).then(
            continueRunActions(sessionId, message, nextContext, i - 1, confidenceThreshold)
          );
        });
      } else {
  ...
  this.runActions = (sessionId, message, context, maxSteps, confidenceThreshold) => {
    ...
    return this.converse(sessionId, message, context).then(
      continueRunActions(sessionId, message, context, steps, confidenceThreshold)
    );
  };
